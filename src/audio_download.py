...
def download_audio(youtube_video, destination):
    download_directory = ""
    ...
    title = helpers.remove_invalid_characters(youtube_video.title)         # Remove invalid characters from the video title.
    full_path = os.path.join(download_directory, f"{title}.mp3")

    helpers.remove_if_exists(full_path)
    helpers.download_stream(audio, f"{title}.mp3") # Download the audio file.
