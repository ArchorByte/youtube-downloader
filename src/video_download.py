...
def download_video(youtube_video, system, resolution, destination):
    ...
    if resolution == None:
        ...
        if resolution not in available_resolutions:
            # We select the highest resolution available if the provided resolution as parameter is not available.
    ...
    helpers.download_stream(video, "video_source.mp4") # Download the video source.
    print()                                    # Separate the two progression bars.
    helpers.download_stream(audio, "audio_source.mp3") # Download the audio source.
